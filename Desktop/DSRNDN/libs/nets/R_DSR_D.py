...
def build_block_2d(incoming, out_channels, filter_size,
    ...
    with tf.variable_scope(scope, default_name=name, values=[net]) as scope:
        net = Squeeze_excitation_layer(incoming, out_dim=incoming.get_shape().as_list()[-1], ratio=0.2,
                                       layer_name='squeeze_layer_')
        # 1st conv layer in residual block
        ...
        net = tf.nn.atrous_conv2d(net, W2, rate=2, padding='SAME') + b2
        ### Add batch nomalization
        if batch_norm:
            net = tf.contrib.layers.batch_norm(net)
        net = tf.nn.relu(net)
