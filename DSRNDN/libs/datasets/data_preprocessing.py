...
def to_tfexample_raw(name, seqLen, seq_feature, pair_feature, label_data):
    return tf.train.Example(features=tf.train.Features(feature={
        ...
        'seq_feature': _bytes_feature(seq_feature),
        'pair_feature': _bytes_feature(pair_feature),
        'label_matrix': _bytes_feature(label_data),
    }))
...
def extract_single(info):
    ...
    # f = codecs.open("feature/pdb25/sequence/" + name.decode() + ".txt", 'w', 'utf-8')
    # f.write(seq.decode())
    #
    # f = codecs.open("feature/pdb25/accuracy/" + name.decode() + ".txt", 'w', 'utf-8')
    # f.write(str(acc))
    #
    # f = codecs.open("feature/pdb25/ss3/" + name.decode() + ".txt", 'w', 'utf-8')
    # f.write(str(ss3))
    #
    # f = codecs.open("feature/pdb25/pssm/" + name.decode() + ".txt", 'w', 'utf-8')
    # f.write(str(pssm))
    #
    # f = codecs.open("feature/pdb25/psfm/" + name.decode() + ".txt", 'w', 'utf-8')
    # f.write(str(psfm))

    # diso = info[b'DISO']
    # print("psfm:", psfm.shape)
    # print("diso:",diso.shape)
    sequence_profile = np.concatenate((pssm, ss3, acc, psfm,cp), axis=1)
    ...
    # datafile = "data/CASP13/"+name.decode()+".txt"
    datafile = "data/CASP12/" + name.decode() + ".txt"
    # datafile = "feature/pdb25" + name.decode() + ".txt"
    f = open(datafile, 'rb')
            ...
            for shard_id in range(num_shards):
                ...
                with tf.python_io.TFRecordWriter(record_filename, options=options) as tfrecord_writer:
                    ...
                    for i in range(start_ndx, end_ndx):
                        ...
                        example = to_tfexample_raw(name, seqLen, seq_feature.tostring(), pair_feature.tostring(),
                                                   label.tostring())
